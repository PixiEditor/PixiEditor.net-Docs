...
Toolsets are fully customizable! See the [Creating custom tools with Brush Engine](https://pixieditor.net/docs/handbook/brushes/brush-tools/) page for details.

![Screenshot of the UI with highlights on toolsets.](./assets/toolsets.png)

### 1. Tool Context Bar

On the left side of the context bar, there are always visible undo and redo arrows. They change the last action made in the app no matter what tool is selected. You can also use shortcuts for that: ```Ctrl + Z``` / ```⌘ + Z``` for undo, and ```Ctrl + Y``` / ```⌘ + Shift + Z``` for redo.
...
-  <span className="pixi-icon icon-create-mask"/> - [Creates a mask](https://pixieditor.net/docs/handbook/layers/masking/) for the selected layer
...
This dropdown lets you choose between different previews of a file. Check [Custom Workspaces](https://pixieditor.net/docs/handbook/node-graph/custom-workspaces/) docs for more info.  
...
-  <span className="pixi-icon icon-play" style="color: #5fad65;"/> - Mini animation player. You can play/pause your animation by clicking on the green button and change the active animation frame with the red indicator underneath.
