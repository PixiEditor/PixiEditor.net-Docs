...
Property sockets are part of the Node Graph system. If you are not familiar with the Node Graph, we recommend that you read the [Getting Started with Nodes](/docs/usage/node-graph/getting-started-with-node-graph) guide first.
...
A texture with a defined width and height. For example [Create Image Node](/docs/usage/node-graph/nodes/create-image) produces a <span style={{color: "#99c47a"}}>⬤</span> `Texture` output, which can be used by other nodes that accept textures, such as the [Modify Image Node](/docs/usage/node-graph/nodes/modify-image).
...
Some Nodes such as [Modify Image Node](/docs/usage/node-graph/nodes/image/modify-image-left) expose a GPU context. In GPU context, the diamond-shaped property sockets work **per pixel**. This means that there is no a single value for the property,
...
In this example, we're modifying the Mona Lisa image with the [Modify Image Node](/docs/usage/node-graph/nodes/image/modify-image-left). This node exposes 2 diamond-shaped sockets:
- `UV`
- `Color`

Notice how the [Modify Image Node](/docs/usage/node-graph/nodes/image/modify-image-left) consists of 2 parts, **Begin Modify Image** and **End Modify Image**, with a blue zone between them. There are 2 Nodes in the blue zone: [Separate Color](/docs/usage/node-graph/nodes/color/separate-color) and [Combine Color](/docs/usage/node-graph/nodes/color/combine-color). By following the connections in this example you should be able to see that we are swapping the red and green channels of the input texture.

Here `Color` is not a single value, instead it's **the color of the currently processed pixel** in the texture. Similarly `UV` is the coordinates of the same pixel. You can think of the pair of [Modify Image Nodes](/docs/usage/node-graph/nodes/image/modify-image-left) as a big loop going over each pixel in the image.
