...
Property sockets are part of the Node Graph system. If you are not familiar with the Node Graph, we recommend that you read the [Getting Started with Nodes](/docs/handbook/node-graph/getting-started-with-node-graph) guide first.
...
Inputs can be in 2 states: **Connected** and **Disconnected**. Disconnected inputs usually expose a value editor to the user, allowing them to change the value of the input.

<Tabs>
  <TabItem label="Connected">
  Connected `Size` input does not expose anything. Its value is determined by the output of the node it is connected to. You can preview the value of the input by hovering over the socket.
...
The simplest example are layers and the **Output Node**. Each layer accepts a <span style={{color: "#ff8c00"}}>⬤</span> `Background` input and returns an `Output` <span style={{color: "#ff8c00"}}>⬤</span>.
...
In this example, chain of 2 painting operations is created.
...
A texture with a defined width and height. For example [Create Image Node](/docs/handbook/node-graph/nodes/create-image) produces a <span style={{color: "#99c47a"}}>⬤</span> `Texture` output, which can be used by other nodes that accept textures, such as the [Modify Image Node](/docs/handbook/node-graph/nodes/modify-image).

### <span style={{color: "#cc5c5c"}}>⬤</span> Filter

Filter object that can be applied via the [Apply Filter Node](/docs/handbook/node-graph/nodes/filter/apply-filter) or directly to the layer. Filters can be used to modify the appearance of the texture, such as applying a blur or a color adjustment.
...
A vector of 2 double precision numbers.

### <span style={{color: "#597513"}}>⬤</span> Vec3D

A vector of 3 double precision numbers.
...
An integer value, which can be used to represent whole numbers. It occupies 4 bytes of memory and can represent any number in the range of -2,147,483,648 to 2,147,483,647
...
A magenta color is used when the property doesn't have a unique color attached. It usually means that property type is not meant to be connected.

### <div style={{display: "inline-block", width: "24px", height: "24px", borderRadius: "50%", background: "conic-gradient(#FF6A72 0%, #FFD84A 25%, #47F0C2 50%, #6A7CFF 75%, #FF6A72 100%)", border: "1px solid #ccc", verticalAlign: "middle"}}></div> Object

A type that can hold any value. All other types inherit from object.

### <span style={{color: "#8F6A36"}}>⬤</span> Document Reference

A reference to another .pixi document.

### <span style={{color: "#e84fb0"}}>⬤</span> Palette

Represents an array of colors. (Added in PixiEditor 2.1.2.1)

## Arrays

Some nodes have long rectangular sockets which look like this:

<Image src={rect} width="140" alt="Array socket"/>

This means that the type of the property is **an array**, arrays are special data types which hold multiple values inside.

[1, 2, 3]

Arrays in PixiEditor are **strongly typed**. It means that you can't mix types in an array.

<Aside type="danger" title='Illegal array*'>
[1, "hello", 5.2] 
</Aside>

*It is possible to include different types inside the array if the base type of the array is [Object](#-object) or if the different types inherit from the base type of the array.
...
They accept the same data types as the regular (circular) sockets, but they are used to represent a special type of data flow.

Diamond sockets are contextful. This means that the data they "hold" is determined by the context of the node they are connected to.
...
Some Nodes such as [Modify Image Node](/docs/handbook/node-graph/nodes/image/modify-image-left) expose a GPU context. In GPU context, the diamond-shaped property sockets work **per pixel**. This means that there is no a single value for the property,
but rather there is a formula that is individually applied to each pixel being processed.
...
In this example, we're modifying the Mona Lisa image with the [Modify Image Node](/docs/handbook/node-graph/nodes/image/modify-image-left). This node exposes 2 diamond-shaped sockets:
- `UV`
- `Color`

Notice how the [Modify Image Node](/docs/handbook/node-graph/nodes/image/modify-image-left) consists of 2 parts, **Begin Modify Image** and **End Modify Image**, with a blue zone between them. There are 2 Nodes in the blue zone: [Separate Color](/docs/handbook/node-graph/nodes/color/separate-color) and [Combine Color](/docs/handbook/node-graph/nodes/color/combine-color). By following the connections in this example you should be able to see that we are swapping the red and green channels of the input texture.

Here `Color` is not a single value, instead it's **the color of the currently processed pixel** in the texture. Similarly `UV` is the coordinates of the same pixel. You can think of the pair of [Modify Image Nodes](/docs/handbook/node-graph/nodes/image/modify-image-left) as a big loop going over each pixel in the image.

For example, consider processing the very first pixel of the image (top-left corner). For that pixel `Color` would be equal to #647360 <span style={{color: "#647360"}}>■</span> and `UV` would be equal to (0, 0). Then the Separate Color Node would 
return:
...
:::caution
In reality, the color values inside GPU Context are normalized to a 0-1 range, but that's irrelevant for this example.
:::
...
This resulting color value and the `UV` are passed to the `End Modify Image` node, which puts the modified pixel back on the texture at the same `UV` coordinate (0, 0).

:::tip
You can modify the UV as well to put the pixel in a different location on the texture. For example, you can use UV to flip the image, or to apply a transformation to the image.
:::

Afterwards the loop would continue processing the next pixel to the right, and so on until the end of the image.
...
**Diamond socket can act the same as circular socket when used in CPU context, but circular socket can never become GPU context aware.**
...
A `VecD` can be implicitly converted to a `Float` (or to any single numeric value) by taking the first value of the vector.
...
A `Float` (or any single numeric value) can be implicitly converted to a `VecD` by creating a vector with the X and Y values equal to the float value.
