...
It is a visual equivalent of [Shader Node](/docs/handbook/node-graph/nodes/effects/shader).
...
You can modify the color any way you want, when you plug it into the Input `Color` of [End Modify Image](/docs/handbook/node-graph/nodes/image/modify-image-right), you'll set the final color at the current UV coordinates (not modified ones, but active in GPU context).
...
Check out [GPU Context](/docs/handbook/node-graph/property-sockets#gpu-context) documentation to learn more about how GPU context works. There is an example of how to use Modify Image pair.
