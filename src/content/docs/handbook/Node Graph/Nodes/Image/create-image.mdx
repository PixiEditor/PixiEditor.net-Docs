...
node: 
  ...
  inputs:
      ...
      - name: Content Matrix
        ...
        default: "Identity"
      - name: Disallow Auto Scaling
        type: Boolean
        description: Tells PixiEditor not to scale the image when rendering on partial resolutions (when zooming out in the viewport). Toggle on if zooming the viewport causes the graph to break or look weird.
        default: "false"
  outputs:
      - name: Image
        type: Texture
