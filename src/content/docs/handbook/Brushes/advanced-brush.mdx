...
<Aside variant="tip" title="Before you start">
  If you haven't already, check out the [Creating custom brushes - Basic](/docs/handbook/brushes/creating-brushes) guide first. It covers the basics of creating custom brushes and will help you understand the concepts used in this guide.
</Aside>
...
- [ ] A basic understanding of the [Node Graph](/docs/handbook/node-graph/getting-started-with-node-graph)
- [ ] Basic knowledge of [creating custom brushes](/docs/handbook/brushes/creating-brushes)
</List>
...
We'll use [Stroke Info](/docs/handbook/node-graph/nodes/brushes/stroke-info/) node for that. 
...
2. Now, create a [Stroke Info](/docs/handbook/node-graph/nodes/brushes/stroke-info/) node and [Apply Mask](/docs/handbook/node-graph/nodes/filters/apply-mask/) node. Connect the [Target Sample Texture](/docs/handbook/node-graph/nodes/brushes/stroke-info/#target-sample-texture) output of the Stroke Info node to the Image input of the Apply Mask node,
...
3. Create a [Grayscale Filter](/docs/handbook/node-graph/nodes/filters/grayscale-filter/) node and connect Filters output to the Filter input of the Apply Mask node. Finally, connect the output of the Apply Mask node to the Content input in Brush Output node.
...
<Aside variant="tip" title="Blurring and other effects">
    Some filters require extra pixels around the edges to work properly (e.g. blurring). In that case, you can use the Oversample input of the Brush Output node to inflate the sample area by a certain amount of pixels. This will give the filter enough pixels to work with and prevent edge artifacts.
    Check out the [Blur tool](https://github.com/PixiEditor/PixiEditor/tree/master/src/PixiEditor/Data/BrushTools/Blur.pixi) for a complete example.
</Aside>

## Blackboard

Check out the [Blackboard](/docs/handbook/node-graph/blackboard/) documentation to learn more about the blackboard.
