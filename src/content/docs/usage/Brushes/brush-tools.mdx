...
```json
{
    "CustomTools": [
        {
            "ToolName": "Gradient",
            "Brush": "path\\to\\brush\\GradientBrush.pixi",
            "Icon": "C:\\Users\\Antho\\AppData\\Local\\PixiEditor\\Configs\\Anthonys Awsme Tools\\TEST.svg",
            "ToolTip": "Inverts colors",
            "DefaultShortcut": "G",
            "ActionDisplays": [
              {
...
```
<i>note in JSON files we use \\\ in place of / or \\ to separate files and folders</i>
<p>Alright we made our tool, but if you loaded up PixiEditor right now nothing would happen. Thats because we need to add it to a `toolset`. To create your own toolset, you could add this just below your `CustomTools` in your JSON file.</p>
...
```
But what if you wanted to add your tool to one of PixiEditor's existing toolset? You could do this.
