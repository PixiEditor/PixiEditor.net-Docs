...
Filter object that can be applied via the [Apply Filter Node](/docs/usage/node-graph/nodes/apply-filter) or directly to the layer. Filters can be used to modify the appearance of the texture, such as applying a blur or a color adjustment.
...
### <span style={{color: "#c984ca"}}>⬤</span> VecD (Vector 2 of double precision numbers)

A vector with 2 double precision numbers.

### <span style={{color: "#597513"}}>⬤</span> Vec3D (Vector 3 of double precision numbers)

A vector with 3 double precision numbers.

### <span style={{color: "#c9b4ca"}}>⬤</span> VecI (Vector 2 of integers)
...
### <span style={{color: "#a473a5"}}>⬤</span> Ellipse Data (Vector Shape)
...
### <span style={{color: "#e1d0e1"}}>⬤</span> Points Data (Vector Shape)

A list of points (VecD).

### <span style={{color: "#f2f2f2"}}>⬤</span> Text Data (Vector Shape)
...
Some Nodes such as [Modify Image Node](/docs/usage/node-graph/nodes/modify-image) expose a GPU context. In GPU context, the property socket with diamond shape work **per pixel**. This means that there is no a single value for the property,
...
In this example, we're modifying the Mona Lisa image with [Modify Image Node](/docs/usage/node-graph/nodes/modify-image). This node exposes 2 diamond-shaped sockets:
- `UV`
- `Color`

Notice how [Modify Image Node](/docs/usage/node-graph/nodes/modify-image) consists of 2 parts: Begin and End with blue zone between them.

There are also 2 Nodes connected to Modify Image: [Separate Color](/docs/usage/node-graph/nodes/separate-color) and [Combine Color](/docs/usage/node-graph/nodes/combine-color). Essentially, in this example, we are swapping red with green channel of the image.
