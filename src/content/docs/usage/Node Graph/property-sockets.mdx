...
A reference to other .pixi document.

## Arrays

Some nodes have a long, rectangular sockets. Which look like this:

<Image src={rect} width="140" alt="Array socket"/>

It means, that the type of the property is **an array**, arrays are special data types which hold multiple values inside.
...
*It is possible to include different types inside the array if the base type of the array is an [Object](#-object) or values inherit from base array type. 
...
They accept the same data types as regular, circular sockets, but they are used to represent special type of data flow.
