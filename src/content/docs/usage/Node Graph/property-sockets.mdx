...
Some Nodes such as [Modify Image Node](/docs/usage/node-graph/nodes/image/begin-modify-image) expose a GPU context. In GPU context, the diamond-shaped property sockets work **per pixel**. This means that there is no a single value for the property,
...
In this example, we're modifying the Mona Lisa image with the [Modify Image Node](/docs/usage/node-graph/nodes/image/begin-modify-image). This node exposes 2 diamond-shaped sockets:
- `UV`
- `Color`

Notice how the [Modify Image Node](/docs/usage/node-graph/nodes/image/begin-modify-image) consists of 2 parts, **Begin Modify Image** and **End Modify Image**, with a blue zone between them. There are 2 Nodes in the blue zone: [Separate Color](/docs/usage/node-graph/nodes/color/separate-color) and [Combine Color](/docs/usage/node-graph/nodes/color/combine-color). By following the connections in this example you should be able to see that we are swapping the red and green channels of the input texture.

Here `Color` is not a single value, instead it's **the color of the currently processed pixel** in the texture. Similarly `UV` is the coordinates of the same pixel. You can think of the pair of [Modify Image Nodes](/docs/usage/node-graph/nodes/image/begin-modify-image) as a big loop going over each pixel in the image.
