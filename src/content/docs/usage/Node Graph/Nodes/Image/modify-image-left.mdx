...
You can modify the color any way you want, when you plug it into the Input `Color` of [End Modify Image](/docs/usage/node-graph/nodes/image/modify-image-right), you'll set the final color at the current UV coordinates (not modified ones, but active in GPU context).
